...
void print_token_kind(TokenKind kind) { printf("%s\n", token_kind_name(kind)); }
...
static int push_token(TokenVec *vec, Token *token) {
    ...
    if (vec->num_of_tokens + 1 > vec->token_capacity) {
        ...
        Token *neue_memeory =
            realloc(vec->data, vec->token_capacity * sizeof(Token));

        if (neue_memeory == NULL) {
            printf("[ERROR] Failed to realloc memory while trying to push "
                   "tokens to TokenVec \n");
            return 2;
        }
    ...
    if (value == NULL) {
        printf("[ERROR] Failed to allocate memory while trying to copy token "
               "value \n");
        return 2;
    }
...
static void handle_char(Lexer *lexer) {
    ...
    if (lexer->current_char == '\'') {
        lexer->err_status =
            gen_lexerror(lexer, LEXER_ERR_INVALID_CHAR_LITERAL_SPAN);
        return;
    }
    ...
    if (lexer->current_char == '\\') {
        ...
        if (!lexer->current_char) {
            lexer->err_status =
                gen_lexerror(lexer, LEXER_ERR_UNTERMINATED_CHAR);
            return;
        }
    ...
    while (lexer->current_char && lexer->current_char != '"') {
        if (lexer->current_char == '\\') {
            ...
            if (!lexer->current_char) {
                free(buffer);
                lexer->err_status =
                    gen_lexerror(lexer, LEXER_ERR_UNTERMINATED_STRING);
                return;
            }
...
static void handle_ident(Lexer *lexer) {
    ...
    if (strcmp(ident, "if") == 0) {
        ...
        push_lexer_token(lexer,
                         (Token){.kind = TOKEN_CONTINUE, .value = ident});
...
static void handle_pipe(Lexer *lexer) {
    if (peek(lexer) == '|') {
        advance(lexer);
        push_lexer_token(lexer,
                         (Token){.kind = TOKEN_PIPE_PIPE, .value = "||"});
        advance(lexer);
    } else {
...
static void skip_block_comment(Lexer *lexer) {
    while (1) {
        if (!lexer->current_char) {
            lexer->err_status =
                gen_lexerror(lexer, LEXER_ERR_UNTERMINATED_BLOCKCOMMENT);
            return;
        }
...
static void handle_minus(Lexer *lexer) {
    if (peek(lexer) == '=') {
        ...
        advance(lexer);
        push_lexer_token(lexer,
                         (Token){.kind = TOKEN_MINUSMINUS, .value = "--"});
        advance(lexer);
    } else {
...
static void handle_colon(Lexer *lexer) {
    if (peek(lexer) == ':') {
        advance(lexer);
        push_lexer_token(lexer,
                         (Token){.kind = TOKEN_COLONCOLON, .value = "::"});
        advance(lexer);
    } else {
...
static void generate_next_token(Lexer *lexer) {
    ...
    default:
        if (isdigit((unsigned char)lexer->current_char)) {
            handle_number(lexer);
        } else if (isalpha((unsigned char)lexer->current_char) ||
                   lexer->current_char == '_') {
            handle_ident(lexer);
        } else {
            lexer->err_status =
                gen_lexerror(lexer, LEXER_ERR_INVALID_CHARACTER);
        }
    }
}
...
LexResult generate_tokens(const char *str_input) {
    ...
    while (lexer.current_char) {
        ...
        if (lexer.err_status.status != LEXER_OK) {
            return (LexResult){.tokens = lexer.tokens,
                               .error = lexer.err_status};
        }
    }
